...
const TopChartCard = ({ song, i, isPlaying, activeSong, handlePauseClick, handlePlayClick }) => {
  ...
  const coverArt = song?.attributes?.artwork?.url.replace('{w}x{h}', '440x440') || '/no-image.png';
  // Use unique ID for active song comparison
      ...
      <div className="flex-1 flex flex-row justify-between items-center">
        ...
        <div className="flex-1 flex flex-col justify-center mx-3">
          ...
          <Link to={`/artists/${artistId}`}>
            <p className="text-base text-gray-300 mt-1">{artistName}</p>
          </Link>
        </div>
      </div>
      <PlayPause
    ...
    <div ref={divRef} className="xl:ml-6 ml-0 xl:mb-0 mb-6 flex-1 xl:max-w-[500px] max-w-full flex flex-col">
      ...
      <div className="w-full flex flex-col mt-8">
        ...
        >
          {topPlays.map((song) => {
            const artistId = song?.relationships?.artists?.data?.[0]?.id || '';
            const artistImage = song?.attributes?.artwork?.url.replace('{w}x{h}', '440x440') || '/no-image.png';
